...
- `vision-hotfix-all` 通过 `INTERFACE_HORIZON_RUNTIME_DEPS` 声明动态加载插件；消费目标使用通用 `horizon_install_runtime_deps` helper staging 普通 runtime DLL 和 provider 插件，不在 example 中维护 DLL 特例名单。
