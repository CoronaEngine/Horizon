...
template<typename T>
[[nodiscard]] inline horizon::dsl::uint resolved_soa_type_size() noexcept {
    return static_cast<horizon::dsl::uint>(
        horizon::core::detail::compile_time_soa_storage_bytes<T>(1u, horizon::core::global_storage_policy()));
}

template<typename T>
[[nodiscard]] inline horizon::dsl::uint resolved_soa_stride(horizon::dsl::uint stride = 0u) noexcept {
    return static_cast<horizon::dsl::uint>(
        horizon::core::detail::compile_time_soa_stride<T>(horizon::core::global_storage_policy(), stride));
}

template<typename T>
[[nodiscard]] inline horizon::dsl::uint resolved_aos_type_size() noexcept {
    return static_cast<horizon::dsl::uint>(
        horizon::core::detail::compile_time_resolved_layout_size<T>(horizon::core::global_storage_policy()));
}

template<typename T>
[[nodiscard]] inline horizon::dsl::uint resolved_aos_stride(horizon::dsl::uint stride = 0u) noexcept {
...
#define OC_MAKE_ATOMIC_SOA_VAR(TemplateArgs, TypeName)                                         \
    TemplateArgs struct horizon::dsl::SOAViewVar<TypeName, TBuffer> {                               \
    public:                                                                                    \
    ...
    private:                                                                                   \
        ...
        horizon::dsl::Uint view_size_{};                                                            \
        horizon::dsl::Uint offset_{};                                                               \
        horizon::dsl::uint stride_{};                                                               \
        horizon::dsl::uint type_size_{};                                                            \
                                                                                               \
    public:                                                                                    \
        SOAViewVar() = default;                                                                \
        explicit SOAViewVar(const TBuffer &buffer,                                             \
                            const horizon::dsl::Uint &view_size = horizon::dsl::InvalidUI32,             \
                            const horizon::dsl::Uint &ofs = 0u, horizon::dsl::uint stride = 0u)          \
            : buffer_var_(buffer),                                                             \
              view_size_(horizon::dsl::min(view_size,                                               \
                                      buffer.template size_in_byte<horizon::dsl::uint>())),         \
              offset_(ofs),                                                                    \
              stride_(horizon::dsl::resolved_soa_stride<atomic_type>(stride)),                      \
              type_size_(horizon::dsl::resolved_soa_type_size<atomic_type>()) {}                    \
                                                                                               \
        template<typename Index>                                                               \
        requires horizon::dsl::is_integral_expr_v<Index>                                            \
        [[nodiscard]] horizon::dsl::Var<atomic_type> read(Index &&index) const noexcept {           \
        ...
        template<typename Index>                                                               \
        requires horizon::dsl::is_integral_expr_v<Index>                                            \
        [[nodiscard]] horizon::dsl::Var<atomic_type> at(Index &&index) const noexcept {             \
            return buffer_var_->template load_as<atomic_type>(offset_ +                        \
                                                              OC_FORWARD(index) * type_size_); \
        }                                                                                      \
                                                                                               \
        template<typename Index>                                                               \
        requires horizon::dsl::is_integral_expr_v<Index>                                            \
        [[nodiscard]] horizon::dsl::Var<atomic_type> &at(Index &&index) noexcept {                  \
            return buffer_var_->template load_as<atomic_type>(offset_ +                        \
                                                              OC_FORWARD(index) * type_size_); \
        }                                                                                      \
                                                                                               \
        template<typename Index>                                                               \
        requires horizon::dsl::is_integral_expr_v<Index>                                            \
        void write(Index &&index, const horizon::dsl::Var<atomic_type> &val) noexcept {             \
            buffer_var_->store(offset_ + OC_FORWARD(index) * type_size_, val);                 \
        }                                                                                      \
                                                                                               \
        template<typename int_type = horizon::dsl::uint>                                            \
        [[nodiscard]] horizon::dsl::Var<int_type> size_in_byte() const noexcept {                   \
...
#define OC_MAKE_ATOMIC_SOA_VIEW(TemplateArgs, TypeName)                                                 \
    TemplateArgs                                                                                        \
    requires horizon::dsl::is_byte_buffer_view_v<TBuffer>                                                    \
    struct horizon::dsl::SOAView<TypeName, TBuffer> {                                                        \
    public:                                                                                             \
        static_assert(is_valid_buffer_element_v<TypeName>);                                             \
        using atomic_type = TypeName;                                                                   \
        static constexpr AccessMode access_mode = SOA;                                                  \
        ...
        [[nodiscard]] TBuffer view() const noexcept {                                                   \
            return TBuffer{buffer_view_.handle(),                                                       \
                           ...
                           buffer_view_.total_size()};                                                  \
                                                                                                        ...
                                                                                                        \
        [[nodiscard]] horizon::dsl::uint size_in_byte() const noexcept {                                     \
            return view_size_ / stride_ * type_size_;                                                   \
        }                                                                                               \
    };
...
OC_MAKE_ATOMIC_SOA_VAR_VIEW(template<typename TBuffer>, horizon::dsl::uint)
OC_MAKE_ATOMIC_SOA_VAR_VIEW(template<typename TBuffer>, horizon::dsl::ulong)
OC_MAKE_ATOMIC_SOA_VAR_VIEW(template<typename TBuffer>, float)
OC_MAKE_ATOMIC_SOA_VAR_VIEW(template<typename TBuffer>, int)
OC_MAKE_ATOMIC_SOA_VAR(template<typename T OC_COMMA horizon::dsl::uint N OC_COMMA typename TBuffer>,
                       horizon::dsl::Vector<T OC_COMMA N>)
OC_MAKE_ATOMIC_SOA_VIEW(template<typename T OC_COMMA horizon::dsl::uint N OC_COMMA typename TBuffer>,
                        horizon::dsl::Vector<T OC_COMMA N>)

///#region make structure soa

#define OC_MAKE_SOA_VAR_MEMBER(field_name) horizon::dsl::SOAViewVar<decltype(std::declval<struct_type &>().field_name), TBuffer> field_name;

#define OC_MAKE_SOA_VAR_MEMBER_CONSTRUCT(field_name)                                                                                      \
    field_name = horizon::dsl::SOAViewVar<decltype(std::declval<struct_type &>().field_name), TBuffer>(buffer_var, view_size, offset, stride); \
...
#define OC_MAKE_STRUCT_SOA_VAR(TemplateArgs, S, ...)                                    \
    TemplateArgs struct horizon::dsl::SOAViewVar<S, TBuffer> {                               \
    public:                                                                             \
        using struct_type = S;                                                          \
    ...
    public:                                                                             \
        ...
        explicit SOAViewVar(const TBuffer &buffer_var,                                  \
                            horizon::dsl::Uint view_size = InvalidUI32,                      \
                            horizon::dsl::Uint offset = 0u,                                  \
                            horizon::dsl::uint stride = 0u) {                                \
            view_size = horizon::dsl::min(buffer_var.template size_in_byte<horizon::dsl::uint>(), \
                                     view_size);                                        \
        ...
        template<typename Index>                                                        \
        requires horizon::dsl::is_integral_expr_v<Index>                                     \
        [[nodiscard]] horizon::dsl::Var<struct_type> read(Index &&index) const noexcept {    \
            return horizon::dsl::outline(                                                    \
                [&] {                                                                   \
                    horizon::dsl::Var<struct_type> ret;                                      \
        ...
        template<typename Index>                                                        \
        requires horizon::dsl::is_integral_expr_v<Index>                                     \
        void write(Index &&index, const horizon::dsl::Var<struct_type> &val) noexcept {      \
            horizon::dsl::outline(                                                           \
                [&] {                                                                   \
        ...
        [[nodiscard]] horizon::dsl::Var<int_type> size_in_byte() const noexcept {            \
            return horizon::dsl::outline(                                                    \
                [&] {                                                                   \
                    horizon::dsl::Var<int_type> ret = 0;                                     \
...
#define OC_MAKE_STRUCT_SOA_VIEW_MEMBER(member_name) \
    horizon::dsl::SOAView<decltype(std::declval<struct_type &>().member_name), TBuffer> member_name;
...
#define OC_MAKE_STRUCT_SOA_VIEW(TemplateArgs, S, first_member, ...)                    \
    TemplateArgs                                                                       \
    requires horizon::dsl::is_byte_buffer_view_v<TBuffer>                                   \
    struct horizon::dsl::SOAView<S, TBuffer> {                                              \
    public:                                                                            \
        using struct_type = S;                                                         \
...
#define OC_MAKE_ARRAY_SOA_VIEW(TemplateArgs, TypeName, ElementType)          \
    TemplateArgs struct horizon::dsl::SOAView<TypeName, TBuffer> {                \
    public:                                                                  \
        ...
        static constexpr bool is_atomic = false;                             \
                                                                             \
    private:                                                                 \
        horizon::dsl::array<horizon::dsl::SOAView<element_type, TBuffer>, N> array_{}; \
                                                                             \
    public:                                                                  \
        SOAView() = default;                                                 \
...
OC_MAKE_ARRAY_SOA_VIEW(template<horizon::dsl::uint N OC_COMMA typename T OC_COMMA typename TBuffer>,
                       horizon::dsl::array<T OC_COMMA N>, T)
...
public:
    ...
    explicit AOSViewVar(const TBuffer &buffer, const Uint &view_size = InvalidUI32,
                        Uint ofs = 0u, horizon::dsl::uint stride = 0u)
        : buffer_var_(buffer), offset_(std::move(ofs)),
                    type_size_(horizon::dsl::resolved_aos_type_size<T>()),
                    stride_(horizon::dsl::resolved_aos_stride<T>(stride)) {
        (void)view_size;
    }
