...
public:
    ...
    template <typename VF, typename FF>
        requires(!std::is_convertible_v<VF, std::string>) && (!std::is_convertible_v<FF, std::string>) && (!std::is_same_v<std::remove_cvref_t<VF>, std::vector<uint32_t>>) && (!std::is_same_v<std::remove_cvref_t<FF>, std::vector<uint32_t>>)
    ...
    {
        autoBindEntries_.push_back({ &proxy.boundResource_,
                                     0, 0,
      ...
      FS::template OutputBindings<RasterizerPipelineBase>
{
    ...
    RasterizerPipeline(uint32_t multiviewCount = 1,
                       const std::source_location& sourceLocation = std::source_location::current())
        : RasterizerPipelineBase(VS::spirv, FS::spirv, multiviewCount, sourceLocation), VSRes(static_cast<RasterizerPipelineBase*>(this)), FSOut(static_cast<RasterizerPipelineBase*>(this))
    {
    }

    RasterizerPipeline(const RasterizerPipeline& other)
        : RasterizerPipelineBase(other), VSRes(static_cast<RasterizerPipelineBase*>(this)), FSOut(static_cast<RasterizerPipelineBase*>(this))
    {
    }

    RasterizerPipeline(RasterizerPipeline&& other) noexcept
        : RasterizerPipelineBase(std::move(other)), VSRes(static_cast<RasterizerPipelineBase*>(this)), FSOut(static_cast<RasterizerPipelineBase*>(this))
    {
    }
      ...
      CS::template Bindings<ComputePipelineBase>
{
    ...
    ComputePipeline(const std::source_location& sourceLocation = std::source_location::current())
        : ComputePipelineBase(CS::spirv, sourceLocation), CSBindings(static_cast<ComputePipelineBase*>(this))
    {
    }

    ComputePipeline(const ComputePipeline& other)
        : ComputePipelineBase(other), CSBindings(static_cast<ComputePipelineBase*>(this))
    {
    }

    ComputePipeline(ComputePipeline&& other) noexcept
        : ComputePipelineBase(std::move(other)), CSBindings(static_cast<ComputePipelineBase*>(this))
    {
    }
