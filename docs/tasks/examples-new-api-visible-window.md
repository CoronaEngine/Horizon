...
<!-- TASK_DOCS_EXAMPLES_NEW_API_VISIBLE_WINDOW_ZH_CN_SHA256: f8cf3af1d07a32cc983feee48a75cb71f68e3523015879f7f6580040c4ac6fac -->
...
- If removing or narrowing `frameSubmitMutexes` exposes `VUID-vkCmdDraw-None-09600` / `VUID-vkCmdBeginRendering-pRenderingInfo-09592`, do not add the example lock back to hide the problem. First confirm that the `ResourceSubmissionTracker` guard covers `VulkanCommandEncoder::encode()` through `Queue::submit()` so `ImageWrap::image_layout` follows actual submit order.
...
- `frameSubmitMutexes` should only protect `latestRenderReceipts` reads and writes. Do not hold that lock while doing render submit, display present, `HardwareExecutor::wait()`, or any queue/device wait.
...
- After a multithreaded `default` fix, run bounded repeat validation: several 16-window 10-20 second launch/close smokes plus one longer run. Success means natural exit after `WM_CLOSE`, no force-kill, no CRT window, and no `VUID`, `SYNC-HAZARD`, or `VK_ERROR` in diagnostics.
