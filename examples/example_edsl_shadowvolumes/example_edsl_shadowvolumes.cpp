...
struct SvSceneShared
{
    ...
    Float4 ambient;
    Float4 diffuse;
    Float4 specular_shininess;
    Float4 fog;
    Float4 color;
    Float4 params; // xy: 屏幕分辨率
};
...
struct SvSceneVertOut
{
    Float3 v_normal;
    Float3 v_view;
};

struct SvVolumeParams
{
    Float4x4 view_proj;
    Float4 params; // xy: 屏幕分辨率
};

void run_example_edsl_shadowvolumes()
{
    glfwInit();
    ...
    SvSceneShared sp;
    SvScenePC model_pc;
    model_pc.model.as_push_constant();
    auto sv_scene_vert = [&](Float3 inPosition, Float3 inNormal) {
        auto mvp = mul(sp.proj_view, model_pc.model);
        auto model_view = mul(sp.view_matrix, model_pc.model);

        Aggregate<SvSceneVertOut> out;
        position() = mul(mvp, Float4(inPosition, 1.0f));
        out->v_normal    = normalize(mul(model_view, Float4(inNormal,0.f))->xyz());
        out->v_view      = mul(model_view, Float4(inPosition, 1.0))->xyz();
        return out;
    };

    Texture2D<ktm::fvec4> ambient_out_color = final_output_image;
    auto sv_ambient_frag = [&](Aggregate<SvSceneVertOut> in) {
        auto z         = length(in->v_view);
        auto fogFactor = clamp(Float(1.0f) / exp2(sp.fog->w * sp.fog->w * z * z * Float(1.442695f)), 0.0f, 1.0f);
        ambient_out_color << Float4(sp.ambient->xyz() * sp.color->xyz() * fogFactor, 1.0f);
    };

    Texture2D<ktm::fvec4> depthval_out_color = scene_depth_image;
    auto sv_depthval_frag = [&](Aggregate<SvSceneVertOut> in) {
        depthval_out_color << Float4(position()->z, 0.0f, 0.0f, 1.0f);
    };

    SvVolumeParams vp;
    auto sv_volume_vert_glsl = [&](Float3 inPosition) {
        position() = mul(vp.view_proj, Float4(inPosition, 1.0f));
    };

    Texture2D<ktm::fvec4> sceneDepth = scene_depth_image;
    Texture2D<ktm::fvec4> volume_out_color = count_image;
    auto sv_volume_frag = [&] {
        auto uv = position()->xy() / vp.params->xy();
        auto scene_z = texture(sceneDepth, uv)->x;
        $IF (position()->z > scene_z)
            $DISCARD;
        ...
        volume_out_color << Float4(sign_value, 0.0f, 0.0f, 0.0f);
    };

    Texture2D<ktm::fvec4> shadowCount = count_image;
    Texture2D<ktm::fvec4> lit_out_color = final_output_image;
    auto sv_lit_frag = [&](Aggregate<SvSceneVertOut> in) {
        auto uv = position()->xy() / sp.params->xy();
        auto count = texture(shadowCount, uv)->x;
        Float visibility;
        $IF (abs(count) < 0.5f)
            visibility = 1.0f;
        $ELSE
        ...
        auto viewDir = -normalize(in->v_view);
        auto toLight = sp.light_pos_vs->xyz() - in->v_view;
        auto ld      = normalize(toLight);

        auto ndotl   = dot(n, ld);
        auto reflected = 2.0f * ndotl * n - ld;
        auto rdotv   = dot(reflected, viewDir);
        auto diff    = max(0.0f, ndotl);
        auto spec    = step(0.0f, ndotl) * pow(max(0.0f, rdotv), sp.specular_shininess->w);

        auto dist = max(length(toLight), sp.light_pos_vs->w);
        auto attn = 50.0f * pow(dist, -2.0f);
        auto rgb   = (diff * sp.diffuse->xyz() + spec * sp.specular_shininess->xyz()) * sp.light_rgb->xyz() * attn;

        auto z         = length(in->v_view);
        auto fogFactor = clamp(Float(1.0f) / exp2(sp.fog->w * sp.fog->w * z * z * Float(1.442695f)), 0.0f, 1.0f);

        lit_out_color << Float4(rgb * sp.color->xyz() * visibility * fogFactor, 1.0f);
    };

    // pass 1: ambient
    horizon::RasterizerPipelineDesc ambient_desc;
    ambient_desc.blend_enabled = false;
    horizon::RasterizerPipeline ambient_rasterizer(sv_scene_vert, sv_ambient_frag, ambient_desc);
    ...
    horizon::RasterizerPipeline depthval_rasterizer(sv_scene_vert, sv_depthval_frag, depthval_desc);
    ...
    horizon::RasterizerPipeline volume_rasterizer(sv_volume_vert_glsl, sv_volume_frag, volume_desc);
    ...
    horizon::RasterizerPipeline lit_rasterizer(sv_scene_vert, sv_lit_frag, lit_desc);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        auto record_scene = [&](auto& pipeline) {
            ...
            sp.diffuse = to_edsl_vec4(diffuse_color);
            sp.specular_shininess = to_edsl_vec4(specular_shininess);
            sp.fog = to_edsl_vec4(fog_params);
        ...
        if (!volume_positions.empty())
        {
            ...
            if (!volume_indirect_cmds.empty())
            {
                horizon::HardwareBuffer volume_indirect_buffer = horizon::HardwareBuffer::from_bytes(
                    ...
                    horizon::BufferUsage_TransferDst | horizon::BufferUsage_Indirect,
                    "example_edsl_shadowvolumes.volume_indirect");
