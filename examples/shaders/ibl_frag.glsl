...
// 辅助函数取自 bgfx examples/common/shaderlib.sh。
...
vec3 calcLambert(vec3 cdiff, float ndotl)
{
    return cdiff * ndotl;
}
...
float specPwr(float gloss)
{
    return exp2(10.0 * gloss + 2.0);
}
...
void main()
{
    ...
    float inReflectivity = fsp.params0.y;
    float inGloss        = fsp.params0.x;

    // 反射率：metalness / specular 两种工作流
    vec3 refl;
    if (fsp.misc.z == 0.0)
    {
    ...
    // 环境贴图由 cmft 预滤波，radiance 使用 mip [1..6]
    float mip = 1.0 + 5.0 * (1.0 - inGloss);

    vec3 vr = 2.0 * ndotv * nn - vv; // 等价于 -reflect(vv, nn)
    ...
    color = color * exp2(fsp.params0.z);
    outColor = vec4(toFilmic(color), 1.0);
}
