...
{
    if (device.value != 0)
        throw std::logic_error("example_glsl fixed queue resolver supports only the main device.");
    ...
    if (found == queues.end())
        throw std::runtime_error("example_glsl could not resolve a graphics queue.");
