...
{
    // Match bx::mtxSRT (column-major, ZYX Euler, scale → rotate → translate).
...
void run_example_gpudrivenrendering()
{
    ...
    // Match bgfx: pow(2, floor(log2(count))+1) — scan size for stream compaction.
    ...
    // Untouched by stream compaction — used when Hi-Z is disabled but indirect draw stays on.
    ...
    bool hiz_enable = true; // default: current full Hi-Z → occlude → compact path
